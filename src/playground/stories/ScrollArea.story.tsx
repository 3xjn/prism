...
function ScrollAreaStoryCanvas({ controls: currentControls }: { readonly controls: ScrollAreaStoryControls }): React.ReactElement {
	...
	for (let index = 1; index <= 14; index += 1) {
		items.push(
			<Box key={`item-${index}`} width={direction === "horizontal" ? 160 : "100%"} bg="background.surface" radius="sm" p="sm" borderColor="border.subtle">
				<Text text={`Scrollable row ${index}`} color="text.primary" width="100%" />
			</Box>,
		);
	}
		...
		<StoryCanvas>
			<Box width="100%" bg="background.surface" radius="md" p="lg">
				<Stack width="100%" gap="md">
					<Text text="ScrollArea" size="lg" weight={700} color="text.primary" />
					<Text
